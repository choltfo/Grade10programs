...
proc DRAW (x,y,i,c:int)
    ...
    for a : 1 .. i
        var xA : int := Rand.Int(-10,10) + sign(mX-x)*20
        var yA : int := Rand.Int(-10,10) + sign(mY-y)*20
        Draw.Line(x,y,x+xA,y+yA,(c mod 6)+9)%(((x xor xA) xor (y xor yA)) mod 71) + 104)
        Add(x+xA,y+yA)
        View.Update()
    end for
end DRAW
...
var metaLayer : int := 1
loop
    DRAW(floor(maxx/2),floor(maxy/2),2,red)
    var layer := 2
    loop
        Mouse.Where(mX, mY, mB)
        for i : 1.. upper(curDotsX)
            DRAW(curDotsX(i),curDotsY(i),layer,layer+metaLayer)
        end for
