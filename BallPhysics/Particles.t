...
var coss : array 1..360 of real
var sins : array 1..360 of real
...
import Particle, particleBurst, coss, sins
...
procedure InitAngular (x,y,maxVel:real, numOfP,Colour,size,TTLMin,TTLMax : int)
    for i : 1 .. numOfP
        new particles, upper (particles)+1
        particles(upper(particles)).col := Colour
        particles(upper(particles)).size := size
        particles(upper(particles)).x := x
        particles(upper(particles)).y := y
        var heading : int := Rand.Int(1,360)
        particles(upper(particles)).xVel := maxVel*coss(heading)
        particles(upper(particles)).yVel := maxVel*sins(heading)
        particles(upper(particles)).TTL := Rand.Int (TTLMin,TTLMax)
        particles(upper(particles)).maxTTL:=particles(upper(particles)).TTL
    end for
end InitAngular
...
PS -> InitAngular (300,200,20,100,yellow,5,5,40)
